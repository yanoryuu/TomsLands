...
namespace Utage
{
	...
	public class SampleChapterTitle : MonoBehaviour
	{
		...
		IEnumerator LoadChaptersAsync(int chapterIndex)
		{
			...
			{
				//オートセーブデータをロード（非同期拡張があればコルーチン経由で待つ）
				if (this.Engine.SaveManager is IAdvSaveManagerAsync saveAsyncExtension)
				{
					if (this.Engine.SaveManager.IsAutoSave)
					{
						//yield returnはtry/catchで囲めないため、ToCoroutineCatchException()で例外を検知する。
						yield return saveAsyncExtension.ReadSaveDataAsync(
								this.Engine.SaveManager.AutoSaveData, this.destroyCancellationToken)
							.ToCoroutineCatchException(
								e =>
								{
									//失敗時の例外処理はここに書く
			...
			{
				//起動シーケンス（AdvEngine.CoBootInit）と同じInitSystemSaveDataAsyncを使う。
				//例外処理は内部で完結するためここでの追加対応は不要
				//（拡張したい場合はAdvEngineと同じGameObjectへIAdvSaveExceptionHandlerの実装を追加すること）
				if (this.Engine.SystemSaveData is IAdvSystemSaveDataAsync)
				{
					yield return this.Engine.InitSystemSaveDataAsync();
				}
				else
				{
