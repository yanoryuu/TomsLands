...
> **2026-09-20 更新（v3 出荷済み・全項目達成）**: 本書の課題はすべて解消した。
> 需要→価格は2層モデル（適正値 × 相場の揺らぎ）で、ボラのクラスタリングは Heat 変調で解決。
> 実測 σ 0.017 / kurt 4.71 / ボラ集中 0.209 / トレンド持続 −0.04 / 張り付き 0。
> 詳細は `Docs/Market_Price_v2_Design.md` §10〜§11。
