...
> **2026-09-20 更新（v2 実装・出荷済み）**: 「需要が価格の方向に効かない」問題は v2 の2層モデルで解消した。
> 高需要銘柄は基準の 1.19×、中需要は 1.00× で均衡し、陳列 ON/OFF が価格差として現れる。
> 実測・残課題は `Docs/Market_Price_v2_Design.md` §10。v2 で残るのはボラのクラスタリング（|r|acf1 0.08、目標 0.15〜0.30）。
