...
> **2026-08 更新（複数設置・選択式製造機・9ビルド）**
> - 同一マシンは**複数台設置可**になった（枠の上限のみ）。設置は台単位（PlacementId）で管理され、
>   詳細パネルの「設置済み一覧」から台を選んで撤去・生産切替できる
> - 自動鍛造炉は**選択式**: 台ごとに生産する武具を選べる。毎朝「製造予算(G)」ぶん進捗が貯まり、
>   選択武具の基準価格に達するごとに1個完成（高い武具ほど遅い＝自動バランス）
> - コンテンツ（レリック23種・マシン9種）と設備画面のUIは生成・配置・コミット済み。
>   生成用の一時ツール(BuildContentSetup)は役目を終えたため削除した（アセットはリポジトリに含まれる）
